        ...
        uint32_t size)
{
    ...
    while(size_left)
    {
        ...
        ucode[12] = QSPIF_QIO_FAST_READ_CMD;  // 0xEB
        ...
        ucode_u32[1] = SPI_CMD_SOT(0);
        ucode_u32[2] = SPI_CMD_TX_DATA(8*1, 0, 0);
        ucode[12] = QSPIF_READ_CMD; // 0x03
        ucode_u32[4] = SPI_CMD_TX_DATA(8*3, 0, 0);
        ucode[20] = (curr_addr >> 16 )& 0xFFUL;
        ucode[21] = (curr_addr >> 8) & 0xFFUL;
        ucode[22] = curr_addr & 0xFFUL;
        ucode_u32[6] = SPI_CMD_RX_DATA(curr_size*8, 0, 0);// use 4 lines to recv
        ucode_u32[7] = SPI_CMD_EOT(1);
#endif
        ...
        pi_spi_receive_with_ucode(qspi_dev, (void*)l2_buff, (curr_size)*8,
                SPI_LINES_FLAG | PI_SPI_CS_AUTO, QSPIF_RCV_UCODE_SIZE,
                ucode);
      ...
      const void *data, uint32_t size)
{
    ...
    if((flash_addr & 0xFF) && (((flash_addr & 0xFF)+size_left) > 0x100))
    {
        ...
        ucode[12] = QSPIF_QIO_PAGE_PROG_CMD;//0x38;
        ucode_u32[4] = SPI_CMD_TX_DATA(8*3, 1, 0);
        ucode[20] = ((flash_addr+curr_pos) & 0x00FF0000)>>16;
        ucode[21] = ((flash_addr+curr_pos) & 0x0000FF00)>>8;
        ucode[22] = ((flash_addr+curr_pos) & 0x000000FF)>>0;
        ucode_u32[6] = SPI_CMD_TX_DATA(curr_size*8, 1, 0);
#endif
#else             // single line
        ucode_u32[0] = pi_spi_get_config(qspi_dev);
        ucode_u32[1] = SPI_CMD_SOT(0);
        ucode_u32[2] = SPI_CMD_TX_DATA(8*4, 0, 0);
        ucode[12] = QSPIF_PAGE_PROG_CMD ;//0x02;
        ucode[13] = ((flash_addr+curr_pos) & 0x00FF0000)>>16;
        ucode[14] = ((flash_addr+curr_pos) & 0x0000FF00)>>8;
        ucode[15] = ((flash_addr+curr_pos) & 0x000000FF)>>0;
        ucode_u32[4] = SPI_CMD_TX_DATA(curr_size*8, 0, 0);
#endif
