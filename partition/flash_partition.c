                                    ...
                                    uint8_t *nbr_of_entries)
{
    ...
    {
        PI_LOG_ERR("part", "Table argument is NULL");
...
// Alloc table containing header
    ...
    if(table == NULL)
    {
        PI_LOG_ERR("part", "Unable to allocate partition table in L2.");
        rc = PI_ERR_L2_NO_MEM;
        goto mount_error;
    }
    ...
    {
        PI_LOG_ERR("part", "Unable to allocate table offset variable in L2.");
        rc = PI_ERR_L2_NO_MEM;
        goto mount_error;
    }
    ...
    {
        PI_LOG_ERR("part", "Partition table offset not found in flash.");
        rc = PI_ERR_NOT_FOUND;
        goto mount_error;
    }
    
    PI_LOG_TRC("part", "Partition table offset 0x%lx", *table_offset_l2);
...
//print_partition_header(&table->header);
    ...
    {
        PI_LOG_ERR("part", "Partition table header magic number error\n");
        rc = PI_ERR_NOT_FOUND;
        goto mount_error;
    }
    ...
    {
        PI_LOG_ERR("part", "Partition table format version missmatch: flash version %u != BSP version %u\n",
    ...
    {
        PI_LOG_ERR("part", "Unable to allocate partition table entries.");
        rc = PI_ERR_L2_NO_MEM;
        goto mount_error;
    }
    ...
    if(table->header.crc_flags)
    {
        ...
        if(rc != PI_OK)
        {
            PI_LOG_ERR("part", "Partitions table verification failed.\n");
